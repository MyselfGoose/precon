...
export default function QuoteForm() {
  ...
  if (sent) return <div className="success-card"><div className="ico">✓</div><h2>We have your request.</h2><p>Thanks for sharing the project details. A PreCon Ext team member will review the scope and follow up with a clear next step. For urgent questions, call <a href="tel:+18005550100">{BRAND.phone}</a>.</p><Link className="btn btn-primary" href="/">Back to home</Link></div>;
  return <form className="form" onSubmit={submit}>
    ...
    {error&&<p className="form-error" role="alert">{error}</p>}<div className="field full"><button className="btn btn-primary" type="submit" style={{width:'100%'}}>Start the conversation</button><span className="hint" style={{textAlign:'center'}}>Form preview — submission handling will be connected before launch.</span></div>
  </form>;
}
