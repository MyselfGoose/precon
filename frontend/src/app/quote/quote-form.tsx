...
export default function QuoteForm() {
  ...
  return <form className="form" onSubmit={submit} noValidate aria-describedby="form-status">
    ...
    <div className="field"><label htmlFor="q-trade">Divisions or services needed</label><select id="q-trade" name="trade" multiple size={5} aria-invalid={Boolean(errors.trade)} aria-describedby={errors.trade ? 'error-trade' : 'hint-trade'}>{TRADES.map(t=><option key={t.slug}>Div {t.div} — {t.name}</option>)}<option>General construction estimate</option><option>Industrial / public projects</option><option>Trade-specific estimate</option><option>Architectural drawings</option><option>MEP / structural engineering</option><option>BIM / visualization</option><option>Property acquisition discussion</option><option>Full project support</option></select><span className="hint" id="hint-trade">Hold Ctrl / Cmd to select several.</span>{errors.trade&&<span className="form-error" id="error-trade">{errors.trade}</span>}</div>
    ...
    <div className="field full"><div className="consent"><div className="consent-title">Consent — required before preparing</div><div className="check"><input required type="checkbox" id="c-contact" name="contact-consent"/><label htmlFor="c-contact"><span className="req">Required</span><br/><b>I agree to be contacted about this request.</b> CSI & Design may email or call me to discuss the services I have requested.</label></div><div className="check"><input type="checkbox" id="c-marketing" name="marketing-consent"/><label htmlFor="c-marketing"><span className="opt">Optional</span><br/><b>Send me occasional updates</b> about estimating, preconstruction, and project resources.</label></div><p className="consent-fine">By preparing this brief you agree to our <Link href="/privacy">Privacy Policy</Link> and <Link href="/terms">Terms of Service</Link>. No message or file is sent by this browser-only form.</p></div>{errors['contact-consent']&&<span className="form-error" id="error-contact-consent">{errors['contact-consent']}</span>}</div>
